 ...
 */
import { callGeminiAPI, callGroqAPI, callHuggingFaceAPI, callOpenAIAPI, callOpenRouterAPI, streamGeminiAPI, streamGroqAPI, streamHuggingFaceAPI, streamOpenAIAPI, streamOpenRouterAPI, } from "./callers.js";
 ...
 */
async function callProviderWithModelChain(provider, message, apiKey) {
    const meta = getProviderMeta(provider);
    const referer = process.env.APP_URL || "https://multi-ai-chat-hub.vercel.app";
    let lastError;
    for (const model of meta.models) {
        try {
            switch (provider) {
                case "gemini":
                    return await callGeminiAPI(message, apiKey, model, markRateLimited);
                case "groq":
...
async function* streamProviderWithModelChain(provider, message, apiKey) {
    const meta = getProviderMeta(provider);
    const referer = process.env.APP_URL || "https://multi-ai-chat-hub.vercel.app";
    let lastError;
    for (const model of meta.models) {
...
export async function orchestrateChat(request) {
    ...
    if (provider) {
        ...
        if (isRateLimited(provider)) {
            const timeRemaining = Math.ceil((RATE_LIMIT_COOLDOWN_MS -
        ...
        try {
            const content = await callProviderWithModelChain(provider, message, apiKey);
    ...
    for (const providerName of FALLBACK_ORDER) {
        ...
        if (isRateLimited(providerName))
            continue;
        try {
            const content = await callProviderWithModelChain(providerName, message, apiKey);
            return { content, provider: meta.displayName, success: true };
        }
        catch (error) {
            if (error instanceof ProviderRateLimitError) {
    ...
    return {
        ...
        provider: "None",
        success: false,
        error: formatUserFacingError("AI providers", "All AI providers failed or are unavailable. Please check your API keys."),
    };
}
/**
 ...
 */
export async function* orchestrateChatStream(request) {
    const { message, provider } = request;
    const tryProvider = async function* (providerName) {
        const meta = getProviderMeta(providerName);
        const apiKey = getServerApiKey(providerName);
        if (!apiKey) {
            yield {
        ...
        try {
            for await (const text of streamProviderWithModelChain(providerName, message, apiKey)) {
    ...
    for (const providerName of FALLBACK_ORDER) {
        if (!getServerApiKey(providerName))
            continue;
        if (isRateLimited(providerName))
            continue;
        const meta = getProviderMeta(providerName);
        ...
        try {
            for await (const text of streamProviderWithModelChain(providerName, message, getServerApiKey(providerName))) {
                emittedDelta = true;
                yield { type: "delta", text };
            }
            yield { type: "done", provider: meta.displayName };
