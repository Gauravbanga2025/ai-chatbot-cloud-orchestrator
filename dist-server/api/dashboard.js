...
export default async function handler(req, res) {
    ...
    try {
        ...
        const formattedProviderStats = Object.entries(providerStats).map(([provider, stats]) => ({
            provider,
        ...
        providerEvents.forEach((event) => {
            ...
            providerData[event.provider].timestamps.push(event.timestamp.toISOString());
        });
        const formattedProviderData = Object.entries(providerData).map(([provider, data]) => ({
            provider,
        ...
        const durations = Object.values(sessionDurations).map((s) => (s.last - s.first) / 1000 / 60 // Convert to minutes
        );
        ...
        const performance = {
            ...
            maxDuration: durationsArray.length > 0
        ...
        return res.status(200).json({
            ...
            timeAndTrends: {
                hourlyActivity,
                dailyEvents,
                peakHour: Object.entries(hourlyActivity).reduce((a, b) => (hourlyActivity[a[0]] > hourlyActivity[b[0]] ? a : b), ["0", 0])[0],
            },
            userEngagement: {
